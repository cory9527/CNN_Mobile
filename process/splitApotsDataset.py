...
from process.processimage import circle_crop_final
...
def split(grouped_data, image_dir, target_image_dir, target_label_dir):
    ...
    copyBathImage(train_data_total, image_dir, target_image_dir + "train_images3")
    copyBathImage(validation_data_total, image_dir, target_image_dir + "test_images3")
    write_to_csv(train_data_total, target_label_dir + "train3.csv")
    write_to_csv(validation_data_total, target_label_dir + "test3.csv")
...
def copyImageToTarget(image_dir, target_dir):
    ...
    pre_image = circle_crop_final(image_dir, 10);
    cv2.imwrite(target_path, pre_image)
    # 复制文件
