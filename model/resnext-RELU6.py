...
def ConvBNReLU6(out, in_channels, channels, kernel=1, stride=1, pad=0, num_group=1):
    out.append(nn.Conv2d(in_channels, channels, kernel,
                         stride, pad, groups=num_group, bias=False))
    out.append(nn.BatchNorm2d(channels))
...
class LinearBottleneck(nn.Module):
    def __init__(self, in_channels, channels, t, stride, use_se=True, se_ratio=12, dropout=0.0,
        ...
        if t != 1:
            ...
            ConvBNReLU6(out, in_channels=in_channels, channels=dw_channels)
        else:
...
class ReXNetRELU6(nn.Module):
    def __init__(self, input_ch=16, final_ch=180, width_mult=1.0, depth_mult=1.0, classes=1000,
                 ...
                 dropout_path=0.25,
                 bn_momentum=0.9):
        super(ReXNetRELU6, self).__init__()
        ...
        ConvBNReLU6(features, 3, int(round(stem_channel * width_mult)), kernel=3, stride=2, pad=1)
        ...
        ConvBNReLU6(features, c, pen_channels)
...
if __name__ == '__main__':
    model = ReXNetRELU6(width_mult=1.0, classes=4)
