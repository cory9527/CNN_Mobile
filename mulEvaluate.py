...
from engine import train_one_epoch, evaluate, evaluateMulModels
...
def get_args_parser():
    ...
    parser.add_argument('--data_set', default='MICCAI',
                        choices=['EyePacs', 'messidor1', 'messidor2', 'rfmid', 'apots', 'rsna', 'MICCAI'],
...
def main(args, modelList):
    ...
    if args.eval:
        ...
        test_stats = evaluateMulModels(data_loader_val, modelList, device, use_amp=args.use_amp)
        print(f"kappa of the network on {len(dataset_val)} test images: {test_stats['kappa']:.5f}%")
        return


def setModel(args, model):
    device = torch.device(args.device)
    # model = ReXNetV2(width_mult=3.0, classes=args.nb_classes, drop_path=args.drop_path)
    # model.load_state_dict(torch.load('F:/wei/NN-MOBILENET/rexnet_3.0.pth'), strict=False)
